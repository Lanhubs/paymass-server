...
export class TransactionService {
    static calculateTransactionFee(currency, amount) {
        ...
        const baseFee = baseFees[currency] || 1.0;
        return Math.max(percentageFee, baseFee);
    }
    static async processIncomingTransaction(walletAddress, fromAddress, amount, currency, txHash) {
        try {
    ...
    static async getTransactionHistory(userId, page = 1, limit = 20, currency, type) {
        try {
            const wallet = await prisma.wallet.findFirst({
    ...
    static async getWalletSummary(userId, currency) {
        try {
            ...
            const walletsWithTransactions = await Promise.all(wallets.map(async (wallet) => {
    ...
    static estimateTransactionFee(currency, amount) {
